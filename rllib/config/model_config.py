...
def vgg16(num_actions, inChannel):
    ...
    modelConfig["fcnet_hiddens"] = [
        [256 * 32 * 32 + 5 * 3 + 4, 64], [64, num_actions]]
...
def jointModel(num_actions, inChannel):
    ...
    modelConfig["conv_activation"] = "relu"
    modelConfig["fcnet_hiddens"] = [
        [128 * 15 * 15 + 5 * 3 + 4, 64], [64, num_actions]]
    modelConfig["fcnet_activation"] = "relu"
...
def sensorModel(num_actions):
    modelConfig = ModelConfigDict()
    modelConfig["fcnet_hiddens"] = [[5 * 3 + 4, 32], [32, 64],
                                    [64, 128], [128, 64], [64, 32], [32, num_actions]]
    modelConfig["fcnet_activation"] = "relu"
...
def registerEnv(num_actions, use_depth, step_length, image_width, image_height):
    env_config = {
        "observation_space": spaces.Dict({
            ...
            "distToGeoFence": spaces.Box(low=-1000, high=1000, shape=(3,), dtype=np.float64),
        }),
...
def registerEnvGetModelConfig(network, num_actions=9, step_length=10, image_width=84, image_height=84):
    if network == "VGG-16":
        registerEnv(num_actions, use_depth=False, step_length=step_length,
        ...
        registerEnv(num_actions, use_depth=True, step_length=step_length,
                    image_width=image_width, image_height=image_height)
        ModelCatalog.register_custom_model('CustomNetwork', CustomNetwork)
